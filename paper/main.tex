...
\usepackage{fancyhdr}
\pagestyle{fancy}\fancyhf{}\fancyhead[L]{Perron vector of a totient Gram matrix}\fancyhead[R]{Version 0.1.0}\fancyfoot[C]{\thepage}
...
\title{The Perron vector of a totient Gram matrix\\and its harmonic approximation}
\author{Jeffery Kline}
\date{Version 0.1.0, September 2026\\[2pt] \small Archived at \href{https://doi.org/10.5281/zenodo.22783896}{doi:10.5281/zenodo.22783896}}
\begin{document}
\maketitle
\begin{abstract}
Take the $n\times n$ matrix $R_n$ whose $(j,k)$ entry counts the integers up to $j/k$ that are coprime to $j$. Its first column holds Euler's totient $\varphi(1),\ldots,\varphi(n)$, and we call its Gram matrix $Q_n=R_n^\top R_n$ a totient Gram matrix. Every entry of $Q_n$ is positive, so $Q_n$ has a unique positive unit eigenvector for its largest eigenvalue, the Perron vector $v_n$. The author's 2020 paper, which introduced these matrices, showed that $Q_n$ has determinant one, with the M\"obius vector as the first column of $Q_n^{-1}$, and observed numerically that $v_n$ is nearly a multiple of the harmonic vector $h_n=(1,1/2,\ldots,1/n)$. It conjectured that
\[
 \lim_{n\to\infty}n\,\bigl\|h_n-(v_n^\top h_n)v_n\bigr\|_\infty=1,
\]
that is, that the part of $h_n$ left after projecting onto $v_n$ has sup norm exactly $1/n$ to first order. We prove this. We also identify the residual: multiplied by $n$, its $k$th coordinate tends to $U(t)$ along $k/n\to1/t$, for an explicit arithmetic function $U$, and the conjecture is equivalent to the inequality $|U(t)|\le1$ for $t\ge1$. That inequality holds by an exact integer computation for $t\le10^6$ and, for larger $t$, by an analytic bound resting on Ramar\'e's explicit estimate for the harmonic M\"obius sum. The same analysis improves two results of the 2020 paper. Its Proposition~8, $(c+o(1))n^3\le\Lambda_n\le(\pi^2/18)n^3+O(n^2)$ for the largest eigenvalue, with $c=\frac13\prod_p(1-2/p^2+1/p^3)\approx0.1427$, becomes $\Lambda_n\sim c\zeta(2)n^3$. Its Corollary~9, $|v_n^\top\muvec_n|\le(c^{-1/2}+o(1))n^{-3/2}$, becomes $v_n^\top\muvec_n\sim n^{-3}/(c\zeta(2)^{3/2})$. Finally, since $\sum_{k\le n}\mu(k)/k=h_n^\top\muvec_n$, one might estimate that sum by replacing $h_n$ with its projection onto $v_n$. An exact identity shows the replacement returns the sum unchanged up to $O(\log n/n)$; no new cancellation follows.
\end{abstract}

\section{Introduction}
Take the $n\times n$ matrix $R_n$ whose $(j,k)$ entry counts the integers up to $j/k$ that are coprime to $j$:
\[
 R_n(j,k)=\#\{1\le a\le\lfloor j/k\rfloor:\gcd(a,j)=1\}.
\]
Its first column holds Euler's totient $\varphi(1),\ldots,\varphi(n)$ and its diagonal is all ones. Its Gram matrix $Q_n=R_n^\top R_n$ has positive entries, so $Q_n$ has a simple largest eigenvalue $\Lambda_n$ and a unique positive unit eigenvector $v_n$, its Perron vector. We call $Q_n$ a totient Gram matrix.

These matrices come from \cite{kline}, where $R_n=D_n^{-1}T_nD_n$ for the divisibility matrix $D_n(i,j)=1_{j\mid i}$ and the lower-triangular all-ones matrix $T_n$. That paper proves
\[
 R_n\muvec_n=e_1,\qquad Q_n\muvec_n=e_1,
\]
for the M\"obius vector $\muvec_n=(\mu(1),\ldots,\mu(n))^\top$, so $Q_n$ has determinant one and the M\"obius vector is the first column of $Q_n^{-1}$. Its Proposition~8 proves $(c+o(1))n^3\le\Lambda_n\le(\pi^2/18)n^3+O(n^2)$ with $c=\frac13\prod_p(1-2/p^2+1/p^3)\approx0.1427$, and its Corollary~9 proves $|v_n^\top\muvec_n|\le(c^{-1/2}+o(1))n^{-3/2}$, where $c^{-1/2}\approx2.6467$. It notes numerically that $v_n$ is nearly a multiple of the harmonic vector $h_n=(1,1/2,\ldots,1/n)^\top$, and closes with the conjecture
\begin{equation}\label{eq:intro-conj}
 \lim_{n\to\infty}n\,\bigl\|h_n-(v_n^\top h_n)v_n\bigr\|_\infty=1.
\end{equation}
The vector inside the norm is what is left of $h_n$ after projecting it onto $v_n$. The conjecture says this residual has sup norm exactly $1/n$ to first order.

\subsection*{Main result}
Theorem~\ref{thm:main} proves \eqref{eq:intro-conj} and describes the residual coordinate by coordinate. Write $r_n=h_n-(v_n^\top h_n)v_n$, $g(a)=\prod_{p\mid a}p/(p+1)$, $G=\prod_p\bigl(1-1/(p(p+1))\bigr)$, $\kappa=3/(2G)$, and, for real $t\ge1$,
\[
 U(t)=t-\kappa\sum_{a\le t}g(a)\Bigl(1-\frac{a^2}{t^2}\Bigr).
\]
Then $n\,r_n(k)\to U(t)$ whenever $k/n\to1/t$, and for each fixed $k$, $n\,r_n(k)\to\rho/k$ for an explicit constant $\rho$ with $|\rho|<6/\pi^2$. Since $U(1)=1$, the conjecture is the same as the inequality $|U(t)|\le1$ for all $t\ge1$. The lower bound in \eqref{eq:intro-conj} is immediate: the last column of $R_n$ is $e_n$, so $n\,r_n(n)\to1$.

\subsection*{Method}
The proof has three parts.
\begin{enumerate}
\item \emph{Matrix} (Lemma~\ref{lem:matrix}). Inclusion--exclusion over the divisors of $j$ gives $R_n(j,k)=\varphi(j)/k+O(\tau(j))$, where $\tau$ counts divisors. So $R_n$ is the rank-one matrix $\varphi_nh_n^\top$ plus an error $E_n$ whose Frobenius norm is $O(n\log^{1/2}n)$, against order $n^{3/2}$ for the main term. Perturbation of a rank-one matrix expresses $v_n$, $\Lambda_n$, and $r_n$ through the correction vector $q=E_n^\top\varphi_n/\|\varphi_n\|_2^2$, with Euclidean error $O(n^{-3/2}\log^3n)$. This part is elementary and uniform in the coordinate.
\item \emph{Arithmetic} (Lemma~\ref{lem:arithmetic}). One has $n\,q_k=-U(n/k)+O(\log^2n/k)$ uniformly in $k$, and for $k\le\log^Bn$ one has $n\,q_k=O(\log^{-A}n)$ for every $A$. Here the prime number theorem enters, in the form $\sum_{k\le x}\mu(k)=O\bigl(x\exp(-a\sqrt{\log x})\bigr)$; nothing unproved is used. The same input gives $U(t)\to0$ and $\int_1^\infty U(t)\,dt/t=1/(4c)-1$, where $c=G/(3\zeta(2))$.
\item \emph{Scalar} (Lemma~\ref{lem:scalar}). On each interval $[m,m+1]$ the function $U$ is convex and given by a closed formula in two partial sums. A program in exact integer arithmetic, with every real constant replaced by an outward-rounded rational interval, checks $U(m)<3/4$ at every integer $2\le m\le10^6$, which by convexity and $U(1)=1$ bounds $U$ above by $1$ on $[1,2]$ and by $3/4$ on $[2,10^6]$, and checks $U>-3/4$ on each half-unit interval $[m,m+\tfrac12]$ and $[m+\tfrac12,m+1]$ up to $10^6$, using that $t-\kappa A_m$ increases and $\kappa B_m/t^2$ decreases across the interval, where $A_m$ and $B_m$ are the two partial sums. For $t\ge10^6$, Ramar\'e's bound $|\sum_{k\le x}\mu(k)/k|\le1/(12\log x)$ for $x\ge687$ \cite{ramare} gives $|U(t)|\le202847/300000<0.68$. Together these cover every real $t\ge1$.
\end{enumerate}

\subsection*{Spectral consequences}
The matrix part alone improves two results of \cite{kline}. Proposition~\ref{prop:cor9} replaces the two-sided bound of Proposition~8 there by the asymptotic $\Lambda_n\sim c\zeta(2)n^3=(G/3)n^3\approx0.2348\,n^3$, and combines the exact identity $\Lambda_n\,v_n^\top\muvec_n=v_n(1)$ with the eigenvector asymptotics to replace the upper bound $(c^{-1/2}+o(1))n^{-3/2}$ of Corollary~9 there by the asymptotic $v_n^\top\muvec_n\sim n^{-3}/(c\zeta(2)^{3/2})=3\sqrt6\,n^{-3}/(\pi G)$. Proposition~\ref{prop:spectrum} bounds the rest of the spectrum by $O(n^2\log n)$.

\subsection*{A cancellation attempt that fails}
Corollary~9 of \cite{kline} came from the decomposition $\sum_{k\le n}\mu(k)/k=h_n^\top\muvec_n=(v_n^\top h_n)(v_n^\top\muvec_n)+r_n^\top\muvec_n$. The first term is now known to be of order $n^{-3}$, so one asks whether the residual correlation $r_n^\top\muvec_n$ can be estimated another way. Section~\ref{sec:obstruction} shows that the residual can be replaced by its scalar profile with summed error $O(\log^3n/n)$, and that, because the profile has controlled variation, known bounds for the Mertens function control the profile correlation $\sum_{k\le n}\mu(k)U(n/k)/n$. An exact convolution identity then shows that the profile correlation equals $\sum_{k\le n}\mu(k)/k$ itself up to $O(\log n/n)$. The approximation is accurate but circular for this purpose. No new estimate for M\"obius sums is claimed.

\subsection*{Prior work and scope}
The matrices, the identities $R_n\muvec_n=e_1$ and $Q_n\muvec_n=e_1$, the eigenvalue bounds, Corollary~9, and the conjecture itself are from \cite{kline}. The totient-sum asymptotic $\sum_{j\le n}\varphi(j)^2\sim cn^3$ is classical. The nearest spectral literature concerns Redheffer's matrix, the $(0,1)$ matrix with $(i,j)$ entry one exactly when $j=1$ or $i\mid j$, whose determinant is the Mertens function. Barrett, Forcade and Pollington~\cite{bfp} showed that all but $\lfloor\log_2n\rfloor+1$ of its eigenvalues equal one and that its spectral radius is asymptotic to $\sqrt n$; Vaughan~\cite{vaughan} refined that analysis; and Cl\'ement and Steinerberger~\cite{cs} showed that its dominant singular vector is close to the explicit vector $k\mapsto\sum_{d\mid k}1/d$. Those results concern a different matrix, with spectral radius of order $\sqrt n$ rather than $n^3$, and none of them addresses $R_n$, $Q_n$, or a sup-norm residual limit. The analytic inputs are an effective Mertens-type bound for $\sum_{k\le x}\mu(k)$, available for example from Lee and Leong \cite{lee}, and Ramar\'e's explicit estimate \cite{ramare}, whose corrigendum \cite{corr} does not affect the theorem used. A bounded literature search, recorded in the repository file \texttt{audit/PRIOR\_ART.md}, found no other resolution of \eqref{eq:intro-conj} and no earlier determination of the constants in Proposition~\ref{prop:cor9}. That is a statement about the searched corpus, not a priority claim.

\subsection*{Evidence and AI assistance}
The argument, the certificate program, and this exposition were developed with AI assistance under the author's direction. One model first proposed the scalar tail bound; another re-implemented it in exact integer arithmetic. The matrix part, the assembled proof, and the certificate were each examined by separate fresh-context model audits, whose reports are preserved in the repository. Such audits are process evidence. They are not peer review, and they do not certify correctness. The certificate is executable, and its receipt can be regenerated with \texttt{code/verify\_reproduction.py}; Appendix~\ref{app:files} lists every companion file.
...
The following theorem settles the conjecture stated at the end of \cite{kline}, immediately after its Corollary~9, and describes the residual coordinate by coordinate.
...
These consequences require Lemma~\ref{lem:matrix}, but not the scalar certificate or the full conjecture. Proposition~8 of \cite{kline} proves $(c+o(1))n^3\le\Lambda_n\le(\pi^2/18)n^3+O(n^2)$, and its Corollary~9 proves $|v_n^\top\muvec_n|\le(c^{-1/2}+o(1))n^{-3/2}$. The next proposition replaces both bounds by asymptotic equivalents.
...
\[
 ...
 \frac3G=4.2586886415.
\]
Thus \eqref{eq:lambda} replaces the two-sided bound of Proposition~8 in \cite{kline}, whose lower constant is this same $c$, by an asymptotic with constant $c\zeta(2)$; and \eqref{eq:sharp} replaces the $n^{-3/2}$ upper bound of Corollary~9 there by a positive asymptotic equivalent of order $n^{-3}$. Even the finite inequality \eqref{eq:exactcor}, without the conjecture or perturbation analysis, already gives $O(n^{-3})$.
...
\end{proof}
\begin{remark}
These statements sharpen Proposition~8 and Corollary~9 of \cite{kline}. No comparison with all subsequent literature is asserted, and no asymptotic equivalent for the second eigenvalue is established.
\end{remark}
...
This document makes no claim of human refereeing or proof-assistant verification. Version 0.1.0 is frozen at tag \texttt{v0.1.0}, commit \texttt{0746f6df69cc7c7b551ef47dffbd8436f0c1a1aa}, and permanently archived under version DOI \href{https://doi.org/10.5281/zenodo.22783896}{10.5281/zenodo.22783896}; the tagged copy of this document carries its prepublication ``release candidate'' date line. The repository's \texttt{README.md} and \texttt{ADMISSION.md} record the release status.
