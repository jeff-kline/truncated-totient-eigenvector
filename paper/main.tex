...
\]
one has $nr_n(1)\to\rho$, $|\rho|<1/\zeta(2)$, and, for each fixed $t\ge1$ and any $k_n/n\to1/t$,
\[
...
\[
 ...
 \|b\|_2=O(n^{5/2}L^{1/2}).
\]
For the first bound, sum $\min(\tau(j)^2,j^2/k^2)$ over $k$ and then use $\sum_{j\le n}j\tau(j)=O(n^2L)$. For the second, use $|(Eh)_j|\le\tau(j)\sum_{k\le n}1/k$ and $\sum_{j\le n}\tau(j)^2=O(nL^3)$. Rank-one perturbation of $ah^\top$ gives the displayed first-order spectral estimates. In particular $\|Ev\|_2=O(n^{1/2}L^{5/2})$.
...
\end{quote}
The line break is typographic; the digest is one string. From the repository root, the reproduction command is
...
\bibitem{ramare} O. Ramar\'e, Explicit estimates on several summatory functions involving the Moebius function, \emph{Mathematics of Computation} \textbf{84} (2015), 1359--1387. \url{https://ramare-olivier.github.io/Maths/mcom2914.pdf}.
\bibitem{corr} O. Ramar\'e, Corrigendum to the preceding article (2019). \url{https://ramare-olivier.github.io/Maths/mcom3449-corrigendum.pdf}.
\bibitem{lee} E. S. Lee and N. Leong, \emph{New explicit bounds for Mertens function and the reciprocal of the Riemann zeta-function}, arXiv:2208.06141v5. \url{https://arxiv.org/abs/2208.06141v5}.
\end{thebibliography}
\end{document}
