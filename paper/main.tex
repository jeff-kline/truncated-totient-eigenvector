...
\maketitle
\begin{abstract}
Take the $n\times n$ matrix whose $(j,k)$ entry counts the integers up to $j/k$ that are coprime to $j$. Its first column holds Euler's totient $\varphi(1),\ldots,\varphi(n)$, and we call its Gram matrix $Q_n$ a totient Gram matrix. Every entry of $Q_n$ is positive, so $Q_n$ has a unique positive unit eigenvector for its largest eigenvalue, the Perron vector $v_n$. The 2020 paper that introduced these matrices showed that $Q_n$ has determinant one and that the M\"obius vector is the first column of $Q_n^{-1}$, observed that $v_n$ is nearly a multiple of the harmonic vector $h_n=(1,1/2,\ldots,1/n)$, and conjectured that what is left of $h_n$ after projecting it onto $v_n$ has sup norm exactly $1/n$ to first order. We prove the conjecture and identify the residual: multiplied by $n$, its $k$th coordinate converges to $U(n/k)$ for an explicit arithmetic function $U$, and the conjecture is equivalent to the inequality $|U(t)|\le1$ for $t\ge1$. That inequality holds by an exact integer computation for $t\le10^6$ and, for larger $t$, by an analytic bound resting on Ramar\'e's explicit estimate for the harmonic M\"obius sum. The same analysis gives the leading constant of the largest eigenvalue, $\Lambda_n\sim(G/3)n^3$ with $G=\prod_p(1-1/(p(p+1)))$, and shows that the M\"obius correlation $v_n^\top\muvec_n$ decays like $n^{-3}$, where the 2020 paper had $n^{-3/2}$. Finally, an exact identity shows that this approximation, applied to the harmonic M\"obius sum $\sum_{k\le n}\mu(k)/k$, returns that sum unchanged up to $O(\log n/n)$. No new cancellation follows from it.
\end{abstract}
...
\]
Then $n\,r_n(k)\to U(t)$ whenever $k/n\to1/t$, and for each fixed $k$, $n\,r_n(k)\to\rho/k$ for an explicit constant $\rho$ with $|\rho|<6/\pi^2$. Since $U(1)=1$, the conjecture is the same as the inequality $|U(t)|\le1$ for all $t\ge1$.
...
\item \emph{Arithmetic} (Lemma~\ref{lem:arithmetic}). One has $n\,q_k=-U(n/k)+O(\log^2n/k)$ uniformly in $k$, and for $k\le\log^Bn$ the quantity $n\,q_k$ is smaller than any power of $\log n$. Here the prime number theorem enters, in the form $\sum_{k\le x}\mu(k)=O\bigl(x\exp(-a\sqrt{\log x})\bigr)$; nothing unproved is used. The same input gives $U(t)\to0$ and $\int_1^\infty U(t)\,dt/t=1/(4c)-1$, where $c=G/(3\zeta(2))$.
\item \emph{Scalar} (Lemma~\ref{lem:scalar}). On each interval $[m,m+1]$ the function $U$ is convex and given by a closed formula in two partial sums. A program in exact integer arithmetic, with every real constant replaced by an outward-rounded rational interval, checks $U(m)<3/4$ at every integer $2\le m\le10^6$ and $U>-3/4$ on every half-integer interval up to $10^6$. For $t\ge10^6$, Ramar\'e's bound $|\sum_{k\le x}\mu(k)/k|\le1/(12\log x)$ for $x\ge687$ \cite{ramare} gives $|U(t)|\le202847/300000<0.68$. Together these cover every real $t\ge1$.
\end{enumerate}
The last column of $R_n$ is $e_n$, which gives $n\,r_n(n)\to1$ and the matching lower bound in \eqref{eq:intro-conj}.
