...
\maketitle
\begin{abstract}
Take the $n\times n$ matrix $R_n$ whose $(j,k)$ entry counts the integers up to $j/k$ that are coprime to $j$. Its first column holds Euler's totient $\varphi(1),\ldots,\varphi(n)$, and we call its Gram matrix $Q_n=R_n^\top R_n$ a totient Gram matrix. Every entry of $Q_n$ is positive, so $Q_n$ has a unique positive unit eigenvector for its largest eigenvalue, the Perron vector $v_n$. The author's 2020 paper, which introduced these matrices, showed that $Q_n$ has determinant one, with the M\"obius vector as the first column of $Q_n^{-1}$, and observed numerically that $v_n$ is nearly a multiple of the harmonic vector $h_n=(1,1/2,\ldots,1/n)$. It conjectured that the part of $h_n$ left after projecting onto $v_n$ has sup norm exactly $1/n$ to first order. We prove this. We also identify the residual: multiplied by $n$, its $k$th coordinate tends to $U(t)$ along $k/n\to1/t$, for an explicit arithmetic function $U$, and the conjecture is equivalent to the inequality $|U(t)|\le1$ for $t\ge1$. That inequality holds by an exact integer computation for $t\le10^6$ and, for larger $t$, by an analytic bound resting on Ramar\'e's explicit estimate for the harmonic M\"obius sum. The same analysis gives the leading constant of the largest eigenvalue, $\Lambda_n\sim(G/3)n^3$ with $G=\prod_p(1-1/(p(p+1)))$, and shows that the M\"obius correlation $v_n^\top\muvec_n$ decays like $n^{-3}$, replacing the 2020 paper's upper bound of order $n^{-3/2}$ by an asymptotic equivalent. Finally, since $\sum_{k\le n}\mu(k)/k=h_n^\top\muvec_n$, one might estimate that sum by replacing $h_n$ with its projection onto $v_n$. An exact identity shows the replacement returns the sum unchanged up to $O(\log n/n)$; no new cancellation follows.
\end{abstract}
...
\]
for the M\"obius vector $\muvec_n=(\mu(1),\ldots,\mu(n))^\top$, so $Q_n$ has determinant one and the M\"obius vector is the first column of $Q_n^{-1}$. It bounds $\Lambda_n$ between $0.1427\,n^3$ and $(\pi^2/18)n^3$, proves in its Corollary~9 that $|v_n^\top\muvec_n|\le(2.6467+o(1))n^{-3/2}$, notes numerically that $v_n$ is nearly a multiple of the harmonic vector $h_n=(1,1/2,\ldots,1/n)^\top$, and closes with the conjecture
...
The matrix part alone sharpens the estimates of \cite{kline}. Proposition~\ref{prop:cor9} gives $\Lambda_n/n^3\to G/3\approx0.2348$, where the 2020 paper had bounds a factor of four apart, and combines the exact identity $\Lambda_n\,v_n^\top\muvec_n=v_n(1)$ with the eigenvector asymptotics to get $v_n^\top\muvec_n\sim3\sqrt6\,n^{-3}/(\pi G)$. This replaces the $n^{-3/2}$ upper bound of Corollary~9 by an asymptotic equivalent of order $n^{-3}$. Proposition~\ref{prop:spectrum} bounds the rest of the spectrum by $O(n^2\log n)$.
