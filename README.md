...
**Release state: DRAFT.** This repository is being prepared under the
project's [public research standard](https://jeff-kline.github.io/posts/research-program/index.html).
No stable archive, tag, or DOI exists yet. Admission under that standard will
be a project release decision, not peer review or a correctness certificate.
...
matrices, proved that `Q_n mu_n = e_1` for the Möbius vector `mu_n`, so that
`det Q_n = 1`, and conjectured that the positive unit dominant eigenvector
`v_n` of `Q_n` satisfies
...
> ```
>
> and for fixed `k` it converges to `rho/k` with `|rho| < 6/pi^2`. The
> conjecture is equivalent to `|U(t)| <= 1` for all real `t >= 1`, which
> holds with equality only at `t = 1`.

The proof is unconditional. It uses the prime number theorem in the effective
form `M(x) = O(x exp(-a sqrt(log x)))` and Ramaré's explicit bound
`|sum_{k <= x} mu(k)/k| <= 1/(12 log x)` for `x >= 687`. No unproved
hypothesis is assumed.

## How the proof goes

1. **Matrix step.** `R_n = phi_n h_n^T + E_n` with `||E_n||_F = O(n log^(1/2) n)`,
   against `||phi_n h_n^T||_F` of order `n^(3/2)`. Rank-one perturbation gives
   `v_n`, the dominant eigenvalue `Lambda_n`, and `r_n` in terms of the
   correction vector `q = E_n^T phi_n / ||phi_n||^2`, up to Euclidean error
   `O(n^(-3/2) log^3 n)`.
...
3. **Scalar step.** `U` is convex on each `[m, m+1]` and given by a closed
   ...
   enclosure `U > -3/4` on all `1,999,998` half-integer intervals, using
   outward rational enclosures with denominator `10^30`. For `t >= 10^6`,
   Ramaré's bound gives `|U(t)| <= 202847/300000 < 0.68`.

The last column of `R_n` is `e_n`, which gives `n r_n(n) -> 1` and the
...
- `Lambda_n / n^3 -> G/3 = 0.2348...`, replacing the 2020 sandwich
  `0.1427 n^3 <~ Lambda_n <= 0.5483 n^3`.
...
or priority claim is made; a bounded literature search found no other
resolution of the conjecture.

## Evidence and reproduction

- [paper/main.tex](paper/main.tex) is the authoritative typeset source and
  [paper/main.pdf](paper/main.pdf) the reading copy, built twice and compared
  byte-for-byte.
- [proof/proof-bundle.md](proof/proof-bundle.md) is the complete examined
  argument; `proof/A0*.md` are its component notes. Status labels inside those
...
- J. Kline, *Unital sums of the Möbius and Mertens functions*, J. Integer
  ...
  `R_n mu_n = e_1` and `Q_n mu_n = e_1`, the eigenvalue sandwich, Corollary 9,
...
- O. Ramaré, *Explicit estimates on several summatory functions involving the
  Moebius function*, Math. Comp. 84 (2015), Theorem 1.2, and its 2019
  corrigendum. The PDFs are not redistributed; their URLs and SHA-256 are
  recorded in `audit/frozen-artifacts.json`.
- E. S. Lee and N. Leong, arXiv:2208.06141, for an explicit effective Mertens
  bound.
...
Source locators are in [audit/campaign/LITERATURE.md](audit/campaign/LITERATURE.md).
...
[audit/campaign/](audit/campaign/). Those audits are process evidence. They
...
[RELEASE-PLAN.md](RELEASE-PLAN.md) is the working plan.
